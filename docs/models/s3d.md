...
| `on_extraction`                         | `print`                               | If `print`, the features are printed to the terminal. If `save_numpy`, `save_pickle`, or `save_h5`, the features are saved to either `.npy`, `.pkl`, or `.h5` file.                                  |
| `output_path`                           | `"./output"`                          | A path to a folder for storing the extracted features (if `on_extraction` is `save_numpy`, `save_pickle`, or `save_h5`; the latter saves to one file).                                                              |
